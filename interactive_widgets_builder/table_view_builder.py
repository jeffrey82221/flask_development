...
class TableViewBuilder():
  ...
  def __plot_head_columns(self, columns):
    '''
  ...
  def __plot_project_specific_accordion(self, project_name):
    ...
    accordion = widgets.Accordion(children=[
        self.__build_table_widget(project_specific_table_info.reset_index())
    ],
        selected_index=None,
  ...
  # TODO: make this function more abstract ...
  def __plot_project_specific_member_level_accordion(self, project_name):
    table_info_used_here = TableRanderer.get_project_specific_member_level_table_info(
        project_name)
    accordion = widgets.Accordion(children=[
        self.__build_table_widget(table_info_used_here.reset_index(),
                                  with_subtable=True,
  ...
  def __plot_member_specific_accordion(self, member_name, project_name=None):
    ...
    if project_name != None:
      ...
      member_specific_table_info = TableRanderer.get_member_specific_table_info(
          member_name)
    accordion = widgets.Accordion(children=[
        self.__build_table_widget(member_specific_table_info.reset_index())
    ],
        selected_index=None,
        layout=Layout(border='solid 1px',
                      width='75.5%'))
  ...
  '''
    ...
    header = self.__plot_head_columns(table.columns)
    # Plot Table Content
    if not with_subtable:
      # no sub-table
      vis = self.__plot_non_expandable_table_content(table)
    else:
      # making sub-table
      grids = []
      for row_index in range(row_count):
        ...
        if next_level == 'table':
          ...
          if current_level == 'project':
            accordion = self.__plot_project_specific_accordion(
                project_name=condition)
          else:  # member
            accordion = self.__plot_member_specific_accordion(
          ...
          accordion = self.__plot_project_specific_member_level_accordion(
              project_name=condition)
        grids.append(accordion)
      vis = VBox(grids)
    return VBox([header, vis])

  # TODO: move the following func to table_randerer_interface package
  def fetch_table_info_of_current_level(
          self,
          current_level,
          condition,
          condition_type):
...
# top
  ...
  def build_top_table_widget(self,
                             current_level='project',
                             with_subtable=True,
                             next_level='member',
                             condition=None,
  ...
  '''
    ...
    if current_level == 'table':
      with_subtable = False
      next_level = None
    table_info_of_current_level = self.fetch_table_info_of_current_level(
        current_level, condition, condition_type)
    # building widget from the table
    table_widget = self.__build_table_widget(
