...
class ViewFactory():
  def __init__(self):
    ...
    self.randerer_modes_for_each_view_type = {
  ...
  def build_view(self, view_type, condition=None, platform='jupyter'):
    randerer_modes = self.randerer_modes_for_each_view_type[view_type]
    # create multiple table views, each on a tab.
    table_widget_list = []
    for i, mode in enumerate(randerer_modes):
      tvb = TableViewBuilder(
          TableRandererOrganizer(
              table_randerers=self.randerer_flows[mode],
              level_names=self.level_names_config[mode]))
      table_widget = tvb.build_table_widget(
          conditions=[condition] if condition else [])
      table_widget_list.append(table_widget)
    tab_nest = widgets.Tab()
    tab_nest.children = table_widget_list
    # Setup title on each table
    for i, mode in enumerate(randerer_modes):
      tab_nest.set_title(
          i, '{level_name}-level information'.format(
              level_name=self.level_names_config[mode][0]))
    if platform == 'flask':
